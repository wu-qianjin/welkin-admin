...
import MenuOperateModal, {
  type OperateType,
} from "./modules/menu-operate-modal.vue";
import { getRouteNameByViewPath, type PageEntry } from "./modules/shared";
...
async function fetchAllMenus(): Promise<Api.SystemManage.Menu[]> {
  ...
  for (let page = 2; page <= pages; page += 1) {
    const rest = await fetchGetMenuList({
      ...filters,
      current: page,
      size: 100,
    });
...
type MenuTreeNode = Omit<Api.SystemManage.Menu, "children"> & {
...
const treeData = computed<MenuTreeNode[]>(() => {
  ...
  for (const item of data.value)
    nodes.set(item.id, { ...item, children: undefined });
  ...
  const sortNodes = (list: MenuTreeNode[]) => {
    list.sort(
      (a, b) =>
        a.menuType.localeCompare(b.menuType) ||
        (a.order ?? 0) - (b.order ?? 0) ||
...
/** 页面组件清单直接来自实际视图文件，并以完整 /src/views 路径作为表单值。 */
const viewModules = import.meta.glob("/src/views/**/*.vue");

const allPages: PageEntry[] = Object.keys(viewModules)
  .filter((viewPath) => /\/(?:index|\[[^/]+\])\.vue$/.test(viewPath))
  .map((viewPath) => ({
    routeName: getRouteNameByViewPath(viewPath),
    viewPath,
  }))
  .sort((a, b) => a.viewPath.localeCompare(b.viewPath));
</script>
