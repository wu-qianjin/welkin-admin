...
function getSubmitParams() {
  ...
  // 顶级路由必须携带布局（顶级单页会按 layout.base$view.x 拆分，顶级目录依赖 layout.base 渲染外壳）；
  // 子级路由忽略布局，只保留页面部分，避免生成 layout.base$view.x 破坏嵌套解析。
  const effectiveLayout = model.value.parentId === "0" ? layout || "base" : "";

  params.component = transformLayoutAndPageToComponent(effectiveLayout, routeName);
...
// 目录不挂载页面组件，切换时清掉残留的 page，防止提交时生成 view.* 组件导致路由解析异常。
watch(
